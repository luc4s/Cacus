...
Cacus::Cacus(uint32_t width, uint32_t height, const char **extensionNames,
  ...
  if (extensionCount > 0) {
    ...
    vkEnumerateInstanceExtensionProperties(nullptr, &vkCount,
                                           vkExtensions.data());
  ...
  if (extensionCount > 0) createInfo.ppEnabledExtensionNames = extensionNames;
...
void Cacus::createImage(uint32_t width, uint32_t height, VkFormat format,
...
void Cacus::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width,
                              uint32_t height) {
...
void Cacus::transitionImageLayout(VkImage image, VkFormat format,
...
VkImageView Cacus::createImageView(VkImage image, VkFormat format,
...
void Cacus::loadTexture(const int texWidth, const int texHeight,
...
VkFormat Cacus::findSupportedFormat(const std::vector<VkFormat> &candidates,
...
uint32_t Cacus::findMemoryType(uint32_t typeFilter,
...
void Cacus::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
void Cacus::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                       VkDeviceSize size) {
  VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void Cacus::createMeshBuffers(const std::vector<Vertex> &newVertices,
...
void Cacus::setTransform(const glm::mat4 &model, const glm::mat4 &view,
                         const glm::mat4 &proj) {
  ubo.model = model;
  ubo.view = view;
  ubo.proj = proj;
...
VkSurfaceFormatKHR Cacus::chooseSwapSurfaceFormat(
...
VkPresentModeKHR Cacus::chooseSwapPresentMode(
...
VkExtent2D Cacus::chooseSwapExtent(
...
bool Cacus::isDeviceSuitable(const VkPhysicalDevice &device) const {
  ...
  return indices.isComplete() && swapChainAdequate &&
         supportedFeatures.samplerAnisotropy && extensionsSupported;
}

SwapChainSupportDetails Cacus::querySwapChainSupport(
...
QueueFamilyIndices Cacus::findQueueFamilies(
    const VkPhysicalDevice &device) const {
